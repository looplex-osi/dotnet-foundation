...
public class GroupService : BaseResourceService<Group>
{
  ...
  public GroupService(IList<IPlugin> plugins, IRbacService rbacService, IHttpContextAccessor httpContextAccessor,
    IMediator mediator) : base(plugins, rbacService, httpContextAccessor, mediator)
  {
  }
  ...
  /// <summary>
  /// Apply business rules specific to groups
  /// </summary>
  protected override async Task<Group> ApplyBusinessRulesAsync(Group resource, CancellationToken cancellationToken)
  {
  ...
  /// <summary>
  /// Validate resource specific to groups
  /// </summary>
  protected override async Task<bool> ValidateResourceAsync(Group resource, CancellationToken cancellationToken)
  {
  ...
  {
    return new QueryResource<Group>(page, count, filter, sortBy, sortOrder);
  }
  ...
  {
    return new CreateResource<Group>((Group)resource);
  }
  ...
  {
    return new RetrieveResource<Group>((Guid)id);
  }
  ...
  {
    return new ReplaceResource<Group>((Guid)id, (Group)resource);
  }
  ...
  {
    return new UpdateResource<Group>((Guid)id, (Group)resource, (JArray)patches);
  }
  ...
  {
    return new DeleteResource<Group>((Guid)id);
  }
